...
public class ConvertImpl implements Convert {
	...
	private static final String[] messageByTens = { "", "", "twenty", "thirty", "forty", "fifty", "sixty", "seventy", "eighty",
			"ninety" };
	 ...
	 * @param number to be turned into words
	 * @return the number as words
	 * @throws IllegalArgumentException if {@code value <= Validator.VALUE_MIN} or {@code value <= Validator.VALUE_MAX} 
	 */
	@Override
	...
	/**
	 * A helper method for numbers greater than or equal to 1000. Used to process hundredth group.  For example 555678, the 555 will be handled 
	 * by this method.
	 * @param a number to be changed into words.  Between 1 and 999. 
	 * @param container which holds the number as words
	 * @param the group string : Trillion, Billion, Million, Thousand
	 */
	...
	private void hundreds(int value, StringBuilder translation) {
		if (value / 100 != 0) {
			translation.append(
					messageNumbers[value / 100] + " " + MESSAGE_HUNDRED + ((value % 100 != 0) ? " " : ""));
		}
	}
	
	/**
	 * Processes the 57 in 657
	 * @param a number between 0 to 99
	 * @param container which holds the number as words
	 * @param add an "and" clause to the translation string
	 */
